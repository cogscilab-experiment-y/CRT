...
def check_isdigit(text):
    return "".join(text).replace('.', '', 1).isdigit() and text[0] != "0"
        ...
        # draw trial for answer_type == mouse
            ...
            while clock.getTime() < config["answer_time"]:
                ...
                if event.getKeys(['backspace']):
                    ...
                    key = event.getKeys(allowed_keys)
                    if key and (key[0] in KEY_TRANSLATOR):
                        key[0] = KEY_TRANSLATOR[key[0]]
                    if check_isdigit(answer+key):
                        answer += key
                else:
                    event.getKeys()
                win.flip()
